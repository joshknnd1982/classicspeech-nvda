...
def _github_release(tag="v1.08", with_checksum=True, **extra):
	assets = [{
		...
		"browser_download_url": f"https://github.com/joshknnd1982/classicspeech-nvda/releases/download/{tag}/addon",
		"size": len(ADDON_BYTES),
	}]
	if with_checksum:
		assets.append({
			"name": f"ClassicSpeech-{tag[1:]}.nvda-addon.sha256",
			"browser_download_url": f"https://github.com/joshknnd1982/classicspeech-nvda/releases/download/{tag}/sha",
		})
	data = {
		"tag_name": tag,
		"name": f"ClassicSpeech {tag[1:]}",
		"body": "## What's new\n\n- **Reset** all settings. See [the guide](https://example.com).\n",
		"html_url": f"https://github.com/joshknnd1982/classicspeech-nvda/releases/tag/{tag}",
		"assets": assets,
	}
	data.update(extra)
	return data
...
class UpdateCheckTests(unittest.TestCase):
	...
	def test_repository_comes_from_the_manifest_url(self):
		self.assertEqual(
			self.updates.github_repository("https://github.com/joshknnd1982/classicspeech-nvda"),
			"joshknnd1982/classicspeech-nvda",
		)
	...
	def test_manifest_points_update_checks_at_joshs_github(self):
		...
		self.assertEqual(self.updates.github_repository(url), "joshknnd1982/classicspeech-nvda")
	...
	def test_fetch_reads_the_latest_release_and_explains_failures(self):
		url = self.updates.API_URL.format(repository="joshknnd1982/classicspeech-nvda")
		session = FakeSession({url: FakeResponse(data=_github_release())})
		release = self.updates.fetch_latest_release("1.07", "joshknnd1982/classicspeech-nvda", session=session)
		self.assertEqual(release.version, "1.08")
		...
		for response, text in (
			...
			with self.assertRaises(self.updates.UpdateError) as caught:
				self.updates.fetch_latest_release("1.07", "joshknnd1982/classicspeech-nvda", session=FakeSession({url: response}))
	...
	def _download(self, checksum_text, content=ADDON_BYTES, release=None):
		...
		return self.updates.download_release(release, "1.07", "joshknnd1982/classicspeech-nvda", self.folder, session=session)
	...
	def test_installed_add_on_needs_a_version_and_a_github_page(self):
		...
		try:
			...
			manifest = {"version": "1.07", "url": "https://github.com/joshknnd1982/classicspeech-nvda"}
			...
			self.assertEqual(self.updates.installed_addon(), ("1.07", "joshknnd1982/classicspeech-nvda"))
			manifest["url"] = ""
			self.assertIsNone(self.updates.installed_addon())
		finally:
			if saved is None:
