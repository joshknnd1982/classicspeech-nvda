...
> **Current release:** [ClassicSpeech 1.18](https://github.com/joshknnd1982/classicspeech-nvda/releases/tag/v1.18) says Control+Space in File Explorer's list of files the way JAWS does: "not selected" and then the file's name when you unselect a file, and just "selected" when you select one. 1.17 makes an unread message in Microsoft Outlook's message list start with "unread" again as you move to it; NVDA had asked Outlook before Outlook could answer, and said "unread" only in a second reading of the whole message, which you seldom heard. 1.16 says "selected" when you select an item yourself, such as a file with Control+Space in File Explorer, whatever **List item state reporting** is set to; with its default, Say not selected, you heard only the file's name. 1.15 makes **NVDA+E**, and the "default" that **Announce default button in dialogs** adds, name a dialog's real default button, the one Enter presses from its other controls, even while focus is on another button: OK in NVDA's settings dialogs, Open (a split button) in the Windows file dialog LibreOffice and most programs use, and the default button of message boxes, property sheets, other programs' dialogs and web forms. When nothing reports it, NVDA+E looks at how the dialog's buttons are drawn on the screen, and asks you to turn off NVDA's Screen Curtain when it hides them. 1.14 adds **Give ClassicSpeech messages priority over NVDA speech** on the Misc page, so ClassicSpeech's own messages, such as the speech hook loaded message, are spoken in full and only a key press stops them, and lets a speech and sound scheme replace the sounds NVDA itself plays (browse and focus mode, auto-suggestions, spelling errors, the screen curtain, Remote Access, NVDA starting and exiting) from the new **NVDA sounds** category. 1.13 puts each release's notes inside the add-on, as the `changelog` in its manifest, so NVDA 2026.1 and later show them when you choose **What's new** for ClassicSpeech in the Add-on Store. 1.12 makes the keyboard layout menu NVDA opens while you add an input gesture read its first item, makes every row of the Speech and Sound Schemes tree readable (rows named after NVDA's own words, such as "menu" and "selected", were silent), and offers the two Hotkeys multiple-choice settings as lists of checkboxes, the way NVDA's Document Formatting page offers Spelling or grammar errors. 1.11 makes every Speech and Sound Schemes item do what you configured: an item's voice and sound follow the text they describe however you read it, a voice your synthesizer cannot hear as a speech command is applied to the synthesizer itself, and an object's states are used on the object. An update offer now shows the release notes in a read-only edit box you can read with the arrow keys. 1.09 fixes Reset All ClassicSpeech Settings, which froze NVDA, and asks with a Yes/No dialog before it resets. 1.08 keeps NVDA settings changed while a voice profile's voice is speaking, and makes Cancel in Web / Browse Mode Settings put everything back. 1.07 moved all ClassicSpeech settings into their own file, `%APPDATA%\nvda\ClassicSpeech\settings.ini`, puts NVDA back the way it was when you reset or remove ClassicSpeech, and checks GitHub for updates. See the [1.18](docs/RELEASE-1.18.md), [1.17](docs/RELEASE-1.17.md), [1.16](docs/RELEASE-1.16.md), [1.15](docs/RELEASE-1.15.md), [1.14](docs/RELEASE-1.14.md), [1.13](docs/RELEASE-1.13.md), [1.12](docs/RELEASE-1.12.md), [1.11](docs/RELEASE-1.11.md), [1.09](docs/RELEASE-1.09.md), [1.08](docs/RELEASE-1.08.md) and [1.07](docs/RELEASE-1.07.md) release notes, and the [1.06](docs/RELEASE-1.06.md), [1.05](docs/RELEASE-1.05.md), [1.04](docs/RELEASE-1.04.md), [1.03](docs/RELEASE-1.03.md) and [1.02](docs/RELEASE-1.02.md) notes for earlier changes. The `main` branch remains active development; commits after a release may not yet have completed live NVDA validation.
