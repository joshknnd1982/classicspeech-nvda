...
- **List item state reporting** (Text Processing) for items the user moves to, with a selection change, such as Control+Space, always spoken with its new state;
...
| `RELEASE-1.17.md` | Current release notes, packaged with the add-on; their What's new is the manifest changelog. |
